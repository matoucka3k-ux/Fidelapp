...
export default function Welcome() {
  ...
  const [rewards, setRewards] = useState([
    ...
    { id: 2, name: 'Viennoiserie offerte', pts: 500 },
  ])
  ...
  const addReward = () => {
    setRewards(r => [...r, { id: nextId, name: '', pts: 1000 }])
    setNextId(n => n + 1)
  }

  const updateReward = (id, field, val) =>
    setRewards(r => r.map(x => x.id === id ? { ...x, [field]: val } : x))
      ...
      <div className={styles.content}>
        <div className={styles.inner}>
          ...
          <p className={styles.sub}>
            Plus qu'une étape : configurez vos règles de points. Ça prend 2 minutes — et vous pourrez tout ajuster quand vous voulez depuis votre tableau de bord.
          </p>
          ...
          <div className={styles.etapes}>
            <div className={styles.etape}>
              ...
              <div className={styles.eBody}>
                ...
                <div className={styles.eDesc}>Votre commerce est enregistré</div>
              </div>
              ...
              {rewards.map(r => (
                <div key={r.id} className={styles.rewardRow}>
                  ...
                  <input
                    ...
                    value={r.pts}
                    min="1"
                    onChange={e => updateReward(r.id, 'pts', +e.target.value)}
                  />
                  <span className={styles.unit}>pts</span>
                </div>
              ))}
            </div>
            <button className={styles.btnAddReward} onClick={addReward}>
              + Ajouter une récompense
            </button>
            <p className={styles.configHint}>
              Vous pouvez en ajouter autant que vous voulez depuis "Système de points"
            </p>
          </div>

          <button className={styles.btnBlue} onClick={() => navigate('/dashboard')}>
